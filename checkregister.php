...
if (isset($_POST["signup_submit"])) //checking if came here from click submit
{
    ...
    {
        $sql = " SELECT * FROM users where Mobile=?; ";  //prepare for every new SQL statement (?) here it's SELECT
        ...
        {
            mysqli_stmt_bind_param($stmt, "i", $mobile);  //add s if more than one string; one string because mobile=? has one '?'. Binding mobile to stmt
            ...
            if($resultCheck > 0) 
            {
                header("Location: ../register.php?error=mobile"); //checks if duplicate mobile number exist
                exit();
            }
