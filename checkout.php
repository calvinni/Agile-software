        ...
        else
        {
            ...
            else
            {
                ...
                While ($row = mysqli_fetch_assoc($result))  // Insert into table orderitems for each item
                {
                    ...
                    $new_id = $UserID + 1;
                    $sql_item = "INSERT INTO orderitems (order_id, cart_id, OrderName, OrderQuantity, type) VALUES ('$list_id', '$new_id', '$Cart_Name', '$Cart_Quantity', '$Type');";
...
else if(isset($_POST["pickup"])) //checking if came here from click submit
{
    require 'dbh.php';  //using the $conn variable
    ...
    else
    {
        ...
        While ($row = mysqli_fetch_assoc($result))  // Insert into table orderitems for each item
        {
            ...
            $Cart_Quantity = $row['OrderQuantity'];
            $new_id = $UserID + 1;
            $sql_item = "INSERT INTO orderitems (cart_id, OrderName, OrderQuantity, type) VALUES ('$new_id', '$Cart_Name', '$Cart_Quantity', '$Type');";
            mysqli_query($conn, $sql_item);
            if (mysqli_affected_rows($conn) < 1)    // if insert fail, return to cart
            {
                header("Location: ../cart.php?fail=items");
